...
class bjtc_step5_analyzer: public analyzer{
		public:
				...
				TCanvas * P_fig3();
				TCanvas * P_diff_bjet();
				void stack_plot(TString name, bool );
				void stack_plot2(TString name);
				void fig10(TString name);
				void fig11(TString name);
				void fig12(TString name);
				void fig13(TString name);
				void fig14(TString name);
				void  probe_plots();
				void post_check();
...
void bjtc_step5_analyzer::preprocess(TString name, TString name2){
		...
		float edge = 0.99;
		js_incl_pp_data->area_normalize(0,     edge,    "width");
		js_incl_pp_SystError->area_normalize(0,edge,"width");
		js_bjet_pp_data->area_normalize(0,     edge,     "width");
		js_bjet_pp_SystError->area_normalize(0,edge,"width");
		js_incl_Pb_data->area_normalize(0     ,edge,     "width");
		js_incl_Pb_SystError->area_normalize(0,edge,"width");
		js_bjet_Pb_data->area_normalize(0,     edge,     "width");
		js_bjet_Pb_SystError->area_normalize(0,edge,"width");
		js_bjet_Pb_SystError_forRatio->area_normalize(0, edge,"width");
		js_incl_Pb_SystError_forRatio->area_normalize(0, edge,"width");
		/*
		*/
...
TCanvas*  bjtc_step5_analyzer::fig8(){
		TCanvas * c = new TCanvas("cfig8","", 1800, 1900);
		c->SetMargin(0.18,0.05,0.15,0.15);
		c->Divide(3,3, 0,0);
		gStyle->SetOptStat(0);
		line.SetLineStyle(2);
		...
		TLegend* tl2=new TLegend(0.25,0.65,0.85,0.92); tl2->SetLineColor(0);
		TLegend* tl3=new TLegend(0.4,0.79,0.95,0.96); tl3->SetLineColor(0);
		for(int i=0; i<3; i++){
				c->cd(3-i);
				...
				h = js_incl_pp_SystError->at(0,i);
				hist_style_error(h,kOrange+7, i==2);
				h->SetMarkerStyle(25);
				h->Draw("e2same");
				h = js_incl_pp_data->at(0,i);
				hist_style_error(h,kOrange+7, i==2);
		...
		txt.SetTextFont(42);
		txt.SetTextSize(0.025);
		txt.DrawLatexNDC(0.055, 0.97, "#sqrt{s_{NN}} = 5.02 TeV, PbPb 1.7 nb^{-1}, pp 27.4 pb^{-1}, anti-k_{T} jet (R = 0.4): #font[12]{p}_{T}^{jet} > 120 GeV, |#font[15]{#eta}_{jet}| < 1.6");
		c->cd(4);
		tl2->Draw();
		c->cd(9);
		tl3->Draw();
		...
		c->cd(0);
		TBox bx; bx.SetFillColor(kWhite); bx.SetFillStyle(1001);
		bx.DrawBox(0.36, 0.025, 0.39, 0.045);
		bx.DrawBox(0.66, 0.025, 0.69, 0.045);
		bx.DrawBox(0.97, 0.025, 0.99, 0.045);
		txt.SetTextSize(0.025);
		txt.DrawLatex(0.365, 0.03, "1");
		txt.DrawLatex(0.67 , 0.03, "1");
		txt.DrawLatex(0.975, 0.03, "1");
		return c;
}

TCanvas*  bjtc_step5_analyzer::fig9(){
		TCanvas * c = new TCanvas("cfig9","", 1800, 1300);
		c->SetMargin(0.18,0.05,0.15,0.15);
		c->Divide(3,2, 0,0);
		gStyle->SetOptStat(0);
		line.SetLineStyle(2);
		...
		for(int i=0; i<3; i++){
				c->cd(3-i);
				TH1* h =(TH1*) js_bjet_Pb_SystError->at(0,i)->Clone(Form("js_diff_bjet_err_%d", i));
				TH1* hpb = js_bjet_Pb_SystError->at(0,i);
				TH1* hpp = js_bjet_pp_SystError->at(0,i);
				h->Add(h,hpp, 1, -1);
				TH1* hbe = h;
				//h->Divide(hpp);

				h->SetAxisRange(-3, 2.5, "Y");
				h->SetAxisRange(0., .99, "X");
				hist_style_error(h,kBlue+2, i==2);
				h->GetYaxis()->SetTitle("(PbPb-pp)");
				h->Draw("e2");
				if(i==0) tl->AddEntry(h, "b jets", "pf");

				h = (TH1*) js_bjet_Pb_data->at(0,i)->Clone(Form("js_diff_bjet_%d",i));
				hpb = js_bjet_Pb_data->at(0,i);
				hpp = js_bjet_pp_data->at(0,i);
				h->Add(h,hpp, 1, -1);
				TH1* hb = h;
				//h->Divide(hpp);
				hist_style_data(h,kBlue+2, i==2);
				...
				h = (TH1*) js_incl_Pb_SystError->at(0,i)->Clone(Form("js_diff_incl_err_%d", i));
				hpb = js_incl_Pb_SystError->at(0,i);
				hpp = js_incl_pp_SystError->at(0,i);
				h->Add(h,hpp, 1, -1);
				TH1* hie = h;
				//h->Divide(hpp);
				...
				h =(TH1*) js_incl_Pb_data->at(0,i)->Clone(Form("js_diff_incl_%d",i));
				hpb = js_incl_Pb_data->at(0,i);
				hpp = js_incl_pp_data->at(0,i);
				h->Add(h,hpp, 1, -1);
				TH1* hi = h;
				//h->Divide(hpp);

				c->cd(6-i);
				h = (TH1*) hbe->Clone(Form("js_diff_err_ratio_%d",i));
				h->Divide(hie);
				h->SetAxisRange(-10., 10, "Y");
				h->SetAxisRange(0., .99, "X");
				hist_style_data(h,kBlue+2);
				h->GetYaxis()->SetTitle("Diff_{b}/Diff_{incl}");
				h->Draw("same");

				h = (TH1*) hb->Clone(Form("js_diff_ratio_%d",i));
				h->Divide(hi);
				h->SetAxisRange(0., 3.5, "Y");
				h->SetAxisRange(0., .99, "X");
				hist_style_error(h,kBlue+2, i==2);
...
void bjtc_step5_analyzer::stack_plot(TString name, bool reverse ){
		auto stack_incl_err    = js_incl_err    ->stack("stack_incl_err", reverse);
		auto stack_incl_pp_err = js_incl_pp_err ->stack("stack_incl_pp_err", reverse);
		auto stack_incl        = js_incl        ->stack("stack_incl", reverse);
		auto stack_incl_pp     = js_incl_pp->stack("stack_incl_pp", reverse);

		auto stack_bjet_err    = js_bjet_err    ->stack("stack_bjet_err", reverse);
		auto stack_bjet_pp_err = js_bjet_pp_err ->stack("stack_bjet_pp_err", reverse);
		auto stack_bjet        = js_bjet        ->stack("stack_bjet", reverse);
		auto stack_bjet_pp     = js_bjet_pp->stack("stack_bjet_pp", reverse);

		TCanvas * c = new TCanvas("stack_plot","", 1600, 700);
		//TCanvas * c = new TCanvas("stack_plot","", 1600, 1400);
		//c->Divide(3,2, 0,0);
		c->SetMargin(0.18,0.05,0.15,0.1);
		c->Divide(3,1, 0,0);
		gStyle->SetOptStat(0);
		line.SetLineStyle(2);
		int colb[] = {kBlue+2, kAzure+1, kCyan+1};
		int coli[] = {kRed+3, kRed, kOrange+8};
		TString ptlabel[] = {"p_{T}>1", "p_{T}>2", "p_{T}>4"};
		if(reverse){
				ptlabel[0] = ", 1 < p_{T}^{track}< 2 GeV";
				ptlabel[1] = ", 1 < p_{T}^{track}< 4 GeV"; 
				ptlabel[2] = ", 1 < p_{T}^{track}< 8 GeV"; 
		}
		TLegend* tl=new TLegend(0.52,0.7,0.98,0.98); tl->SetLineColor(0);
		for(int i=0; i<3; i++){
				for(int j=0; j<3; j++){
						c->cd(3-i);
						TH1* h =(TH1*) stack_bjet_err->at(j,i)->Clone(Form("stack_bjet_diff_err_%d",i));
						h->Add(h, stack_bjet_pp_err->at(j,0), 1, -1);
						TH1* hnum_err = (TH1*) h->Clone(Form("stack_ratio_err_%d",i));

						h->SetAxisRange(-100, 100, "Y");
						if(reverse) h->SetAxisRange(-10, 30, "Y");
						h->SetAxisRange(0., .99, "X");
						hist_style_error(h,colb[j], i==2);
						h->GetYaxis()->SetTitle("P(#Deltar)_{Pb}-P(#Deltar)_{pp}");
						if(j==0)h->Draw("e2");
						else h->Draw("samee2");
						if(i==0) tl->AddEntry(h, "b jet "+ptlabel[j], "pf");

						h =(TH1*) stack_bjet->at(j,i)->Clone(Form("stack_bjet_diff_%d",i));
						h->Add(h, stack_bjet_pp->at(j,0), 1, -1);
						hist_style_data(h,colb[j], i==2);
						h->Draw("same");
						TH1* hnum= (TH1*) h->Clone(Form("stack_ratio_%d",i));

						h =(TH1*) stack_incl->at(j,i)->Clone(Form("stack_incl_diff_%d",i));
						h->Add(h, stack_incl_pp->at(j,0), 1, -1);
						hist_style_data(h,coli[j], i==2);
						h->SetMarkerStyle(25);
						h->Draw("same");
						TH1* hden= h;	

						h =(TH1*) stack_incl_err->at(j,i)->Clone(Form("stack_incl_diff_err_%d",i));
						h->Add(h, stack_incl_pp_err->at(j,0), 1, -1);
						hist_style_error(h,coli[j], i==2);
						if(i==0) tl->AddEntry(h, "incl. jet "+ptlabel[j], "pf");
						h->SetMarkerStyle(25);
						h->Draw("samee2");
						TH1* hden_err= h;
				}
...
void bjtc_step5_analyzer::stack_plot2(TString name ){
		bool reverse  = 1;
		auto stack_incl_err    = js_incl_err    ->stack("stack_incl_err", reverse);
		auto stack_incl_pp_err = js_incl_pp_err ->stack("stack_incl_pp_err", reverse);
		auto stack_incl        = js_incl        ->stack("stack_incl", reverse);
		...
		gStyle->SetOptStat(0);
		line.SetLineStyle(2);
		int colb[] = {kAzure+2, kGreen+2,  kRed+2};
		int coli[] = {kAzure+2, kGreen+2,  kRed+2};
		//int colb[] = {kBlue+2, kAzure+1, kCyan+1};
		//int coli[] = {kRed+3, kRed, kOrange+8};
		TString ptlabel[] = {", 1 < p_{T}^{track}< 2 GeV", 
				", 1 < p_{T}^{track}< 4 GeV",
				", 1 < p_{T}^{track}< 8 GeV"};
		TLegend* tl=new TLegend(0.55,0.62,0.98,0.98); tl->SetLineColor(0);
		for(int i=0; i<3; i++){
				for(int j=0; j<3; j++){
						//				if(j==1) continue;	
						c->cd(3-i);
						...
						if(j<2 && i==0) tl->AddEntry((TObject*)0, "", "");
				}
		}
		c->cd(1);
		cms_caption(0.24,0.93,0.07);
...
void bjtc_step5_analyzer::fig10(TString name ){
		TCanvas * c = new TCanvas("stack_plot","", 1600, 700);
		//TCanvas * c = new TCanvas("stack_plot","", 1600, 1400);
		//c->Divide(3,2, 0,0);
		c->SetMargin(0.18,0.05,0.15,0.1);
		...
		gStyle->SetOptStat(0);
		line.SetLineStyle(2);
		int colb[] = {kAzure+2, kRed+2,  kGray+2 };
		int coli[] = {kAzure+2, kRed+2,  kGray+2 };
		//int colb[] = {kBlue+2, kAzure+1, kCyan+1};
		//int coli[] = {kRed+3, kRed, kOrange+8};
		TString ptlabel[] = {", 1 < p_{T}^{track}< 2 GeV", 
				", 2 < p_{T}^{track}< 4 GeV",
		...
		for(int i=0; i<3; i++){
				for(int j=0; j<3; j++){
						...
						TH1* h;
						if(j ==2){
								h =(TH1*) js_bjet_err->at(0,i)->Clone(Form("P_bjet_diff_err_%d_%d",i, j));
								h->Add(js_bjet_err->at(1,i));
								h->Add(h, js_bjet_pp_err->at(0,0), 1, -1);
								h->Add(h, js_bjet_pp_err->at(1,0), 1, -1);
						}
						else {
								h =(TH1*) js_bjet_err->at(j,i)->Clone(Form("P_bjet_diff_err_%d_%d",i,j));
								h->Add(h, js_bjet_pp_err->at(j,0), 1, -1);
						}
						h->SetAxisRange(-7, 22, "Y");
						h->SetAxisRange(0., .99, "X");
						hist_style_error(h,colb[j], i==2);
						...
						if( j ==2){
								h =(TH1*) js_bjet->at(0,i)->Clone(Form("P_bjet_diff_%d_%d",i, j));
								h->Add(js_bjet->at(1,i));
								h->Add(h, js_bjet_pp->at(0,0), 1, -1);
								h->Add(h, js_bjet_pp->at(1,0), 1, -1);
						}else{
								h =(TH1*) js_bjet->at(j,i)->Clone(Form("P_bjet_diff_%d_%d",i,j));
								h->Add(h, js_bjet_pp->at(j,0), 1, -1);
						}
						hist_style_data(h,colb[j], i==2);
						h->Draw("same");

						if( j ==2) {
								h =(TH1*) js_incl->at(0,i)->Clone(Form("P_incl_diff_%d_%d",i, j));
								h->Add(js_incl->at(1,i));
								h->Add(h, js_incl_pp->at(0,0), 1, -1);
								h->Add(h, js_incl_pp->at(1,0), 1, -1);
						}else{ 
								h =(TH1*) js_incl->at(j,i)->Clone(Form("P_incl_diff_%d_%d",i,j));
								h->Add(h, js_incl_pp->at(j,0), 1, -1);
						}
						hist_style_data(h,coli[j], i==2);
						h->SetMarkerStyle(25);
						h->Draw("same");
						...
						if( j ==2){
								h =(TH1*) js_incl_err->at(0,i)->Clone(Form("P_incl_diff_err_%d_%d",i, j));
								h->Add(js_incl_err->at(1,i));
								h->Add(h, js_incl_pp_err->at(0,0), 1, -1);
								h->Add(h, js_incl_pp_err->at(1,0), 1, -1);
						}else {
								h =(TH1*) js_incl_err->at(j,i)->Clone(Form("P_incl_diff_err_%d_%d",i,j));
								h->Add(h, js_incl_pp_err->at(j,0), 1, -1);
						}
						hist_style_error(h,coli[j], i==2);
						if(i==0) tl->AddEntry(h, "incl. jet "+ptlabel[j], "pf");
						h->SetMarkerStyle(25);
...
void bjtc_step5_analyzer::fig12(TString name ){
		auto p_incl_pp = (jtcTH1Player*) js_incl_pp->clone("p_incl_pp");
		auto p_bjet_pp = (jtcTH1Player*) js_bjet_pp->clone("p_bjet_pp");
		//auto p_incl_pp = (jtcTH1Player*) js_incl_pp->contractX("p_incl_pp");
		//auto p_bjet_pp = (jtcTH1Player*) js_bjet_pp->contractX("p_bjet_pp");

		TH1* h = p_incl_pp->at(2,0);
		h->Add(p_incl_pp->at(3,0));
		h->Add(p_incl_pp->at(4,0));
		h->Add(p_incl_pp->at(5,0));
		int n1 = h->GetXaxis()->FindBin(0.4);
		int n2 = h->GetXaxis()->FindBin(0.99);

//
		auto pp = (TH1D*) js_incl_pp->at(3,0)->Clone("pp_incl");
		pp->Add(js_incl_pp->at(4,0));
		pp->Add(js_incl_pp->at(5,0));
		auto central = (TH1D*)js_incl->at(3,0)->Clone("cent_incl");
		central->Add(js_incl->at(4,0));
		central->Add(js_incl->at(5,0));
		
		auto r1 = pp->Integral(1, n1, "width");
		auto r2 = central->Integral(1, n1, "width");
cout<<r1<<" : "<<r2<<endl;
		auto pp_b = (TH1D*) js_bjet_pp->at(3,0)->Clone("pp_bjet");
		pp_b->Add(js_bjet_pp->at(4,0));
		pp_b->Add(js_bjet_pp->at(5,0));
		auto central_b = (TH1D*)js_incl->at(3,0)->Clone("cent_bjet");
		central_b->Add(js_bjet->at(4,0));
		central_b->Add(js_bjet->at(5,0));
		r1 = pp_b->Integral(1, n1, "width");
		r2 = central_b->Integral(1, n1, "width");
cout<<r1<<" : "<<r2<<endl;
//
		
		float c1 = h->Integral(1, n2, "width");
		float c2 = h->Integral(1, n1, "width");
		TString cent[] = {"Cent:0-10%", "Cent:10-30%", "Cent:30-90%"};
		cout<<"pp, incl: [0., 0.4]: "<<c2/c1*100<<"%, [0.4, 1.0]: "<<(c1-c2)/c1*100<<"%"<<endl;
		cout<<"pp, incl: [0., 0.4]: "<<c2<<" GeV, [0.4, 1.0]: "<<(c1-c2)<<" GeV"<<endl;
		h = p_bjet_pp->at(2,0);
		h->Add(p_bjet_pp->at(3,0));
		h->Add(p_bjet_pp->at(4,0));
		h->Add(p_bjet_pp->at(5,0));
		c1 = h->Integral(1, n2, "width");
		c2 = h->Integral(1, n1, "width");
		cout<<"pp, bjet: [0., 0.4]: "<<c2/c1*100<<"%, [0.4, 1.0]: "<<(c1-c2)/c1*100<<"%"<<endl;
		cout<<"pp, bjet: [0., 0.4]: "<<c2<<" GeV, [0.4, 1.0]: "<<(c1-c2)<<" GeV"<<endl;
		for(int i=0; i<3; i++){
			//auto p_incl = (jtcTH1Player*) js_incl->contractX("p_incl");
			//auto p_bjet = (jtcTH1Player*) js_bjet->contractX("p_bjet");
			auto p_incl = (jtcTH1Player*) js_incl->clone("p_incl");
			auto p_bjet = (jtcTH1Player*) js_bjet->clone("p_bjet");
			h = p_incl->at(2,i);
			h->Add( p_incl->at(3,i));
			h->Add( p_incl->at(4,i));
			h->Add( p_incl->at(5,i));
			c1 = h->Integral(1, n2, "width");
			c2 = h->Integral(1, n1, "width");
			cout<<cent[i]<<", inlc: [0., 0.4]: "<<c2/c1*100<<"%, [0.4, 1.0]: "<<(c1-c2)/c1*100<<"%"<<endl;
			cout<<cent[i]<<", inlc: [0., 0.4]: "<<c2<<" GeV, [0.4, 1.0]: "<<(c1-c2)<<" GeV"<<endl;
			h = p_bjet->at(2,i);
			h->Add( p_bjet->at(3,i));
			h->Add( p_bjet->at(4,i));
			h->Add( p_bjet->at(5,i));
			c1 = h->Integral(1, n2, "width");
			c2 = h->Integral(1, n1, "width");
			cout<<cent[i]<<", bjet: [0., 0.4]: "<<c2/c1*100<<"%, [0.4, 1.0]: "<<(c1-c2)/c1*100<<"%"<<endl;
			cout<<cent[i]<<", bjet: [0., 0.4]: "<<c2<<" GeV, [0.4, 1.0]: "<<(c1-c2)<<" GeV"<<endl;
		}
}



void bjtc_step5_analyzer::fig11(TString name ){
		TCanvas * c = new TCanvas("fig11","", 1800, 1900);
		c->SetMargin(0.18,0.05,0.15,0.15);
		c->Divide(3,3, 0,0);
		gStyle->SetOptStat(0);
		line.SetLineStyle(2);
		TLegend* tl=new TLegend(0.4,0.55,0.95,0.93); tl->SetLineColor(0);
		TLegend* tl2=new TLegend(0.25,0.75,0.85,0.92); tl2->SetLineColor(0);
		TLegend* tl3=new TLegend(0.25,0.75,0.95,0.92); tl3->SetLineColor(0);
//		TLegend* tl3=new TLegend(0.4,0.79,0.95,0.96); tl3->SetLineColor(0);
		for(int i=0; i<3; i++){
				c->cd(3-i);
				...
				h->GetYaxis()->SetTitle("P(#Deltar)_{PbPb}-P(#Deltar)_{pp}");
				h->Draw("e2");
				if(i==0) tl3->AddEntry(h, "b jets, 1 < p_{T}^{track} < 4 GeV", "pf");
				...
				hist_style_error(h,kRed+2, i==2);
				h->Draw("e2same");
				if(i==0) tl3->AddEntry(h, "Inclusive, 1 < p_{T}^{track} < 4 GeV", "pf");
		...
		c->cd(0);
		TBox bx; bx.SetFillColor(kWhite); bx.SetFillStyle(1001);
		bx.DrawBox(0.36, 0.022, 0.39, 0.045);
		bx.DrawBox(0.66, 0.022, 0.69, 0.045);
		bx.DrawBox(0.97, 0.022, 0.99, 0.045);
		txt.SetTextSize(0.025);
		txt.DrawLatex(0.365, 0.03, "1");
		txt.DrawLatex(0.67 , 0.03, "1");
		txt.DrawLatex(0.975, 0.03, "1");
...
void bjtc_step5_analyzer::fig13(TString name ){
		TCanvas * c = new TCanvas("fig13","", 1800, 2340);
		c->SetMargin(0.18,0.05,0.15,0.15);
		c->Divide(3,4, 0,0);
		gStyle->SetOptStat(0);
		line.SetLineStyle(2);
...
void bjtc_step5_analyzer::analyze(){
		...
		//auto cfig8 = fig8();
		//cfig8->SaveAs(fig_output+"/figure_nominal_fig3.pdf");
		//auto cfig9 = fig9();
		//cfig9->SaveAs(fig_output+"/figure_supp.pdf");

		//stack_plot("stack_diff.pdf", 0);
		//stack_plot("stack_diff_reverse.pdf", 1);
