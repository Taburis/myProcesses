...
class bjtcSignalProducer : public jtcSignalProducer{
	public : bjtcSignalProducer(TString name0, int npt, int ncent):jtcSignalProducer(name0,npt,ncent){
		 }
		 ~bjtcSignalProducer(){}
...
//		 virtual void sb_correction2(jtcTH1Player *j2);
		 ...
		 ParaSet *ps;
		 bool dorebin = 0, doSbSM = 0; 		
		 bool doIntegral = 0; 		
		 TString *centLabels, *ptLabels;
};

void bjtcSignalProducer::makeMixTable(){
...
void bjtcSignalProducer::produce(){
	sb_ymin = 1.6;
	sb_ymax = 2.4;
	//sb_ymin = 1.6;
	//sb_ymax = 2.2;
...
//	cout<<"nrow: "<<jsig_p1->Nrow()<<" , ncol: "<<jsig_p1->Ncol()<<endl;
	...
	if(doSbCorrection) {
		...
		//if(doSbSM) sb_smth_correction(jsig_p1);
...
jtcTH1Player* bjtcSignalProducer::rebin(TString name, jtcTH1Player *js){
...
void bjtcSignalProducer::sb_smth_correction(jtcTH1Player *j2){
	int ncut = 3;
	float xmin = -3.0, xmax = 3.0, centerleft = -0.05, centerright = 0.05;
	auto deta_sb_p1_origin = j2->projX(_name+"_sig_deta_origin_p1_*_*", sb_ymin, sb_ymax, "e", 0);
	deta_sb_p1_origin->rebin(10);
	auto deta_sb_p1_sm = (jtcTH1Player*) deta_sb_p1_origin->clone(_name+"_sig_deta_p1_sm");
	for(int j=0; j<ncut; j++){
		for(int k=0; k<deta_sb_p1_origin->Ncol(); k++){
			deta_sb_p1_sm->at(j,k)->SetAxisRange(xmin, 2., "X");
			//deta_sb_p1_sm->at(j,k)->SetAxisRange(xmin, xmax, "X");
			deta_sb_p1_sm->at(j,k)->Smooth(1, "R");
			deta_sb_p1_sm->at(j,k)->SetAxisRange(xmin, xmax, "X");
		}
	}
	for(int i=0; i<ncut;++i){
		for(int j=0; j<n2;++j){
			...
			auto h = deta_sb_p1_sm->at(i,j);
			auto h2= j2->at(i,j);
			int n0 = h->FindBin(centerleft);
			int n1 = h->FindBin(centerright);
			int nx0 = h2->GetXaxis()->FindBin(xmin);
			int nx1 = h2->GetXaxis()->FindBin(xmax);
			...
			for(int k=1; k<h2->GetNbinsX()+1; k++){
				if( k<nx0 || k> nx1) continue;
				if( k>=n0 && k<=n1) continue;
				double center = h2->GetXaxis()->GetBinCenter(k);
				int kk = h->GetXaxis()->FindBin(center);
				double cc = double(h->GetBinContent(kk))/scale;
				if (cc ==0) continue;
	...
	auto check = j2->projX(_name+"_sig_deta_origin_p1_*_*", sb_ymin, sb_ymax, "e", 0);
	check->rebin(5);
	auto c = new plotManager();
	c->initSquarePad(_name+"sb_sm", "", ncut-1, deta_sb_p1_origin->Ncol());
	c->addm2TH1(deta_sb_p1_origin, "sideband");
	c->addm2TH1(deta_sb_p1_sm, "Smooth");
//	c->addm2TH1(check, "check");
	c->setXrange(-3.5,3.499);
	c->draw();
	c->drawLegend();
	c->save(out_plot+"/smooth_sbcorr_"+_name+format);
}

void bjtcSignalProducer::sb_correction(jtcTH1Player *j2){
	//float xmin = -2.0, xmax = 1.0, centerleft = -0.1, centerright = 0.1;
	float xmin = -2.9, xmax = 2.9, centerleft = -0.1, centerright = 0.1;
	//float xmin = -2.5, xmax = 2.49, centerleft = -0.1, centerright = 0.1;
	...
	auto fLau = new TF1("fexp", fLaurent, xmin, xmax, 4);
...
bool bjtcSignalProducer::sb_check(TH1* hsig, TH1* hsb, TF1* f){
