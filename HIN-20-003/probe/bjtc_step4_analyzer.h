...
//				contbias = new jtcTH1Player("corrections/contBias_p0_*_*", base->npt, base->ncent);
				...
				trkeff = new jtcTH1Player("corrections/tagTrue_trkEff_nominal_*_*", base->npt, base->ncent);
				...
				//biaseff = new jtcTH1Player("corrections/tagBias_bmc_of_c5sft_smth_*_*", base->npt, base->ncent);
				biaseff = new jtcTH1Player("corrections/tagBias_dmc_c5sft_smth_*_*", base->npt, base->ncent);
		...
		void debug_plot_dr(TString savename,jtcTH1Player*j1, jtcTH1Player*j2,TString lab1="",TString lab2="", int n= 6,int m= 3);
...
void bjtc_step4_analyzer::validation_decontamination(){
	...
	mistagRate[2] = 1-purity->GetBinContent(3);
...
void bjtc_step4_analyzer::closure_in_bMC(){
	auto rs = new jtcTH1Player("correlations_bjetMC_pr_std_c5sft/tagged"+reco_tag(1,1)+"_sig_p1_*_*", base->npt, base->ncent);
	auto is = new jtcTH1Player("correlations_djetMC_std_c5sft/incl"+reco_tag(1,1)+"_sig_p2_*_*", base->npt, base->ncent);
	rs->autoLoad(fstep2);
	is->autoLoad(fstep2);
	auto ref  = new jtcTH1Player("correlations_bjetMC_pr_sube_c5sft/trueB_sube0"+reco_tag(0,0)+"_sig_p2_dr_*_*", base->npt, base->ncent);
	ref->autoLoad(fstep2);
	auto ref_sum  = ref->contractX("ref_spill_sum_*_*");
	float sf[3] = {0.01, 0.01, 0.01};
	auto hist = produce_wf002("bjetMC", rs, is,sf);
	auto probe   =hist.step_spill->contractX("probe_spill_sum_*_*");

	//auto ref2d  = new jtcTH1Player("correlations_bjetMC_pr_sube_c5sft/trueB_sube0"+reco_tag(0,0)+"_sig_p2_*_*", base->npt, base->ncent);
	//ref2d->autoLoad(fstep2);
	//auto ref_err2d  = ref2d->contractX("ref_spill_sum_*_*");
	//ref_err2d->getBkgError();
	//jtcTH1Player* probe_err =(jtcTH1Player*) hist.bkg_error->contractX("probe_bkg_error");
	//auto ref_err  = ref_sum->clone("Ref_closure");
	//((jtcTH1Player*)ref_err)->setDrError(ref_err2d);
	//plot_overlay_uncert("closure_in_bMC", fig_output, probe, "Probe.", ref_sum,"Gen-level", probe_err, ref_err, 0,0.99, 1);
	closurePlot_pTsum("closure_in_bMC",probe, ref_sum,"Probe.","Ref.",-100,900,  1, 3);
}

void bjtc_step4_analyzer::full_closure_test(){
	auto rs = new jtcTH1Player("correlations_djetMC_std_c5sft/tagged"+reco_tag(1,1)+"_sig_p1_*_*", base->npt, base->ncent);
	rs->autoLoad(fstep2);
	auto ns = new jtcTH1Player("correlations_djetMC_std_c5sft/incl"+reco_tag(1,1)+"_sig_p2_*_*", base->npt, base->ncent);
	auto is = new jtcTH1Player("correlations_djetMC_std_c5sft/incl"+reco_tag(1,1)+"_sig_p2_*_*", base->npt, base->ncent);
	//auto ns = new jtcTH1Player("correlations_djetMC_std/negTag"+reco_tag(1,1)+"_sig_p1_*_*", base->npt, base->ncent);
	//ns->autoLoad(fstep2);
	is->autoLoad(fstep2);
	auto ref_decont = new jtcTH1Player("correlations_djetMC_std_c5sft/tagTrue"+reco_tag(1,1)+"_sig_p2_dr_*_*", base->npt, base->ncent);
	auto ref_trk    = new jtcTH1Player("correlations_djetMC_std_c5sft/tagTrue"+reco_tag(1,0)+"_sig_p2_dr_*_*", base->npt, base->ncent);
	auto ref_bias   = new jtcTH1Player("correlations_djetMC_std_c5sft/trueB"+reco_tag(1,0)+"_sig_p2_dr_*_*", base->npt, base->ncent);
	auto ref_jff    = new jtcTH1Player("correlations_djetMC_std_c5sft/trueB"+reco_tag(0,0)+"_sig_p2_dr_*_*", base->npt, base->ncent);
	auto ref_spill  = new jtcTH1Player("correlations_djetMC_sube_c5sft/trueB_sube0"+reco_tag(0,0)+"_sig_p2_dr_*_*", base->npt, base->ncent);
	auto ref_bkg    = new jtcTH1Player("correlations_djetMC_std_c5sft/tagged"+reco_tag(1,1)+"_sig_p2_dr_*_*", base->npt, base->ncent);
	...
	auto probe_trk_sum    = hist.step_trk   ->contractX("probe_trk_sum_*_*");
	auto probe_bias_sum   = hist.step_bias  ->contractX("probe_bias_sum_*_*");
	...
	debug_plot_dr("closure_trk_step",hist.step_trk, ref_trk,"trk step.","tag&true(RG)");
	debug_plot_dr("closure_bias_step",hist.step_bias, ref_bias,"bias step.","true(RG)");
	...
	int ncol = base->ncent;
	closurePlot_pTsum("closure_pTcombined_input_step",probe_input_sum, ref_decont_sum,"input step","tag&true(RR)", -100, 1000,1,ncol);
	closurePlot_pTsum("closure_pTcombined_decont_step",probe_decont_sum, ref_decont_sum,"decont. step","tag&true(RR)", -100, 1000,1,ncol);
	closurePlot_pTsum("closure_pTcombined_trk_step"  ,probe_trk_sum, ref_trk_sum,"trk step.","tag&true(RG)", -100, 1100 , 1,ncol);
	closurePlot_pTsum("closure_pTcombined_bias_step" ,probe_bias_sum, ref_bias_sum,"bias step.","true(RG)", -100, 1100, 1,ncol);
	//closurePlot_pTsum("closure_pTcombined_bkg_step",ref_bkg_sum, probe_bkg_sum,"bkg. step.","tag&true(RR)",-100,900,  1,2);
	closurePlot_pTsum("closure_pTcombined_bkg_step",probe_bkg_sum, ref_bkg_sum,"bkg. step.","tag&true(RR)",-100,900,  1,ncol);
	closurePlot_pTsum("closure_pTcombined_jff_step",probe_jff_sum, ref_jff_sum,"jff. step.","true(GG)"   ,-100, 1200, 1,ncol);
	closurePlot_pTsum("closure_pTcombined_spill_step",probe_spill_sum, ref_spill_sum,"Probe.","Gen-level",-100, 1200, 1,ncol);



	auto sig = new jtcTH1Player("correlations_djetMC_std_c5sft/tagged"+reco_tag(1,1)+"_sig_p2_*_*", base->npt, base->ncent);
	auto sig_ref = new jtcTH1Player("correlations_djetMC_sube_c5sft/trueB_sube0"+reco_tag(0,0)+"_sig_p2_*_*", base->npt, base->ncent);
	sig->autoLoad(fstep2);
...
void bjtc_step4_analyzer::analyze(){
	...
	fstep3 = TFile::Open(step3fname+".root");
