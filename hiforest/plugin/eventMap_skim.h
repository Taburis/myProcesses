...
class eventMap  {
		public : 
				...
				int nRhoBin = 0;
				Double_t rho[jetMax], etaMin[jetMax], etaMax[jetMax];
...
void eventMap::loadRhoInfo(){
		evtTree->SetBranchAddress("nRhoBin", &nRhoBin);
...
double eventMap::getUE(double Eta, double R){
		...
		for(int i = 0; i < nRhoBin; i++)
		{
				...
				double XMin = (max(etaMin[i], Eta - R) - Eta) / R;
				double XMax = (min(etaMax[i], Eta + R) - Eta) / R;
				...
				Result = Result + R * R * (High - Low) * rho[i];
		}
		return Result;
}
