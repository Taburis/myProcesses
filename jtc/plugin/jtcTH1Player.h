...
cout<<"normalizing "<<this->getName()<<endl;
		for(int j=0; j<Ncol(); ++j){
				for(int i=0; i<Nrow(); i++){
						int n1 = this->at(i,j)->GetXaxis()->FindBin(x0);
						int n2 = this->at(i,j)->GetXaxis()->FindBin(x1);
cout<<n1<<"  --------------------  "<<n2<<endl;
...
jtcTH1Player* jtcTH1Player::stack(const char* name, bool reverse= 0){
		// return the jtcTH1Player that summation from i and up. 
		// (need to implement) For the doreverse case, it is the summation for i and below
		jtcTH1Player * res = (jtcTH1Player*) this->clone(name);
		int n = Nrow();
		for(int j=0; j<Nrow(); ++j){
